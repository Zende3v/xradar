...
| POST | `/api/traffic/route {coordinates, aheadM?}` | Bearer ; notre tracé renvoyé à TomTom (points d'appui, 1 tous les 30 m, 1000 max) : portions ralenties **sur notre route** en mètres (`fromM`, `toM`, `level` slow/jam/heavy/closed, `delayS` réparti par longueur quand une section TomTom est coupée), `totalM`, `travelS` / `delayS` (temps TomTom avec trafic, part perdue) ; cache 60 s ; plus les **bouchons des conducteurs** là où ils coûtent plus que TomTom (`source: "crowd"`, le surplus seulement) : signalements « Bouchon » confirmés dans le sens du trajet (2 conducteurs, admin ou sondes ; 1 km, 3 min sans mesure) et ralentissements mesurés par 3 conducteurs (sondes, retard = vitesse médiane contre limitation) ; avec `aheadM` (progression du conducteur), `check` dit si `/api/route/faster` vaut la peine ; 503 sans clé, 502 si TomTom muet (429 = quota) |
